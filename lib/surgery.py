...
def filter_dets_mask(boxes, masks, obj_scores, obj_classes, rel_inds, pred_scores):
    """
    Filters detections....
    :param boxes: [num_box, topk, 4] if bbox regression else [num_box, 4]
    :param obj_scores: [num_box] probabilities for the scores
    ...
    overal_scores, rel_scores_idx = torch.sort(rel_scores_argmaxed.view(-1), dim=0, descending=True)

    rels = rel_inds[rel_scores_idx].cpu().numpy()
    rel_classes = pred_classes_argmax[rel_scores_idx].cpu().numpy()
    objs_labels = obj_classes.data.cpu().numpy()
    obj_scores = obj_scores.data.cpu().numpy()
    boxes_out = boxes.data.cpu().numpy()
    masks_out = masks.data.cpu().numpy()
    return boxes_out, masks_out, objs_labels, obj_scores, rels, rel_classes, overal_scores
