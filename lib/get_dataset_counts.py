...
from dataloaders.pic import PICDataset
from lib.fpn.box_intersections_cpu.bbox import bbox_overlaps
from lib.pytorch_misc import nonintersecting_2d_inds


def get_counts(train_data=PICDataset(mode='train', filter_duplicate_rels=False, use_for_bias=True), must_overlap=True):
    """
    ...
    for img_name in train_data.img_names:
        gt_classes = train_data.img2classes[img_name].copy()
        gt_relations = train_data.img2rels[img_name].copy()
        gt_boxes = train_data.img2boxes[img_name].copy()
