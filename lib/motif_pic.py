...
from lib.surgery import filter_dets_mask
...
def _sort_by_score(im_inds, scores):
    ...
    # img....
    # ~~~~~~~~~~~~~~~~
    #print(scores.shape, rois_per_image.shape, im_inds.shape, rois_per_image[im_inds].shape)
...
class LinearizedContext(nn.Module):
    ...
    def forward(self, obj_fmaps, obj_logits, im_inds, obj_labels=None, box_priors=None, boxes_per_cls=None):
        ...
        #print(obj_embed.shape, pos_embed.shape, obj_fmaps.shape)
...
class RelModel(nn.Module):
    ...
    def __init__(self, classes, rel_classes, mode='sgdet', num_gpus=1, use_vision=True, require_overlap_det=True,
        ...
        self.embed_dim = embed_dim
        self.hidden_dim = hidden_dim
        self.obj_dim = 1024 #2048 if use_resnet else 4096
        self.pooling_dim = 1024 #pooling_dim
        ...
        # self.cls_emb = nn.Embedding(self.num_classes, self.num_classes)
        # self.cls_emb.weight.data = torch.eye(self.num_classes)
    ...
    def forward(self, x, im_sizes, image_offset,
        ...
        if self.training:
            ...
            # 指定rel的gt, roi不发生变化
            rois, labels, rel_labels = proposal_assignments_gtbox(rois.data, gt_boxes.data, gt_classes.data, gt_rels.data, image_offset)
            #boxes = rois[:, 1:]
            pred_boxes = rois[:, 1:]
            pred_masks = gt_masks
            ...
            labels = gt_classes[:, 1]
            rel_labels = None
            pred_dists = Variable(to_onehot(pred_dists.data.long(), self.num_classes))
