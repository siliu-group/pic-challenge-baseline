...
def main(pred_path):
    size = (640, 480)
    method = 'sgg'
    gt = PIC(semantic_path='data/pic/test_gt/semantic',
             instance_path='data/pic/test_gt/instance',
             relation_json='data/pic/test_gt/relations.json',
             mode='gt', size=size)
    pred = PIC(semantic_path=os.path.join(pred_path, 'semantic'),
               instance_path=os.path.join(pred_path, 'instance'),
               relation_json=os.path.join(pred_path, 'relations.json'),
               mode='pred', size=size, top_k=100)
    # pred = PIC(semantic_path='output/'+method+'/semantic',
    #            instance_path='output/'+method+'/instance',
    #            relation_json='output/'+method+'/relations.json',
    #            mode='pred', size=size, top_k=100)
